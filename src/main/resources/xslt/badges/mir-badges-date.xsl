...
<xsl:stylesheet version="3.0"
  ...
  <xsl:template match="doc" mode="badge">
    ...
    <xsl:choose>
      <!-- START save adaptions -->
      <!--
      <xsl:when test="str[@name='mods.dateIssued'] or str[@name='mods.dateIssued.host']">
      -->
      <xsl:when test="str[@name='mods.dateCreated'] or str[@name='mods.dateCreated.host']">
      <!-- END save adaptions -->
        <xsl:call-template name="output-date-badge">
          ...
          -->
          <xsl:with-param name="date" select="str[@name='mods.dateCreated'] | str[@name='mods.dateCreated.host']"/>
          <!-- END save adaptions -->
        </xsl:call-template>
      </xsl:when>
      <!-- START save adaptions -->
      ...
      -->
      <xsl:when test="field[@name='mods.dateCreated'] or field[@name='mods.dateCreated.host']">
      <!-- END save adaptions -->
        <xsl:call-template name="output-date-badge">
          <!-- START save adaptions -->
          <!--
          ...
          -->
          <xsl:with-param name="date" select="field[@name='mods.dateCreated'] | field[@name='mods.dateCreated.host']"/>
          <!-- END save adaptions -->
        </xsl:call-template>
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:template name="output-date-badge">
    ...
    <xsl:call-template name="output-badge">
      ...
      <xsl:with-param name="tooltip" select="mcri18n:translate('mir.date.issued')"/>
      -->
      <xsl:with-param name="link" select="
        concat($ServletsBaseURL, 'solr/find?condQuery=*&amp;fq=mods.dateCreated:%22', string(($date)[1]), '%22')
      "/>
      <xsl:with-param name="tooltip" select="mcri18n:translate('mir.date.created')"/>
      <!-- END save adaptions -->
    </xsl:call-template>
  </xsl:template>
